...
var swiperVideos = new Swiper(".mySwiperVideos", {
    ...
    loop: true,
    noSwiping: true,
    noSwipingClass: 'video-controls-container', // Class that will be added dynamically
    pagination: {
        el: ".swiper-pagination",
        clickable: true,
    },
...
function setupVideoControls() {
    document.querySelectorAll('.swiper-slide video').forEach(video => {
        ...
        // Move native controls to the container
        ...
        // Add events to block propagation
...
function handleVideoInteraction(e) {
    // If click was on a control or the video itself
...
function handleVideoClick(e) {
    // Check if click was on a video control
...
// Fullscreen exit detection and redirect
function setupFullscreenExitRedirect() {
        ...
        try {
            if (!isFullscreenActive()) {
                ...
                if (videosSection) {
                    ...
                    // window.location.hash = '#videos-section';
                }
            }
        } catch (error) {
...
setupVideoControls();
const cleanupFullscreenExitRedirect = setupFullscreenExitRedirect();

// Reapply handlers when Swiper updates
swiperVideos.on('slideChangeTransitionEnd', function() {
    setupVideoClickHandlers();
    setupVideoControls();
});

// Configure fullscreen exit event
